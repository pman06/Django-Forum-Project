#from django.contrib.auth.tokens import PasswordResetTokenGenerator as default_token_gnenerator
...
class PasswordResetTests(TestCase):
    ...
    def test_form_inputs(self):
        '''
        The view must include 2 inputs, email and csrf
        '''
...
class SuccessfulPasswordResetTests(TestCase):
    def setUp(self):
        ...
        User.objects.create_user(username="username", email=email, password="123abcdef")
        url = reverse('password_reset')
        self.response = self.client.post(url, {'email':email})

    def test_redirection(self):
        '''
        A valid form submission should redirect user to the PasswordDone view
        '''
...
class PasswordResetConfirmTests(TestCase):
    ...
    def test_form_inputs(self):
        '''
        The view must container two inputs: csrftoken and two password fields
        '''
